...
def get_avg_info_of_attribute(df, attribute):
    ...
    unique_attribute_values, unique_attribute_array = np.unique(
        attribute_values, return_counts=True
    )
    ...
    for attribute_value in unique_attribute_values:
        ...
        instances_unique_values, instances_unique_counts = np.unique(
        ...
        for i in instances_unique_counts:
            ...
            if j != 0:
                entropy_of_attribute_value = entropy_of_attribute_value - (
                    j * np.log2(j)
                )
        avg_info_of_attribute = avg_info_of_attribute + entropy_of_attribute_value * (
            total_count_in_an_instance / no_of_instances
        )
